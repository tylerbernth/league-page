...
export const getUpcomingDraft = async () => {
    ...
    const [officialDraftRes, picksRes, draftPicksRes] = await waitForAll(
    ...
    const [officialDraft, picks, draftPicks] = await waitForAll(
    ...
    if(officialDraft.status == "complete") {
        ...
        const buildRes = buildConfirmed(officialDraft.slot_to_roster_id, officialDraft.settings.rounds, picks.filter(pick => parseInt(pick.season) == year), draftPicks, officialDraft.type);
        draft = buildRes.draft;
        draftOrder = buildRes.draftOrder;
    }
    ...
    if(players && type != 'auction') {
        ...
        for(const pick of picks) {
            ...
            try {
                ...
                console.error(`Possibly invalid roster ID?: ${pick.roster_id}`, error);
            }
        }
    }
...
const completedNonAuction = ({players, draft, picks, draftOrder, rounds}) => {
    for(const playerData of players) {
        ...
        // Map full object to preserve keeper metadata flags from Sleeper
        draft[playerData.round - 1][playerData.draft_slot - 1] = {
            player,
            is_keeper: playerData.is_keeper || playerData.metadata?.is_keeper || false,
    ...
    for(const pick of picks) {
        if(pick.owner_id == pick.roster_id || pick.round > rounds) continue;
        try {
            const cellIndex = draftOrder.indexOf(pick.roster_id);
            if(cellIndex !== -1 && draft[pick.round - 1][cellIndex]) {
                if(typeof draft[pick.round - 1][cellIndex] === 'object') {
                    draft[pick.round - 1][cellIndex].newOwner = pick.owner_id;
                } else {
                    draft[pick.round - 1][cellIndex] = {
                        player: draft[pick.round - 1][cellIndex],
